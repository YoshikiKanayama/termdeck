...
`DECK_STATUS_CMD` を設定すると、その出力の1行目がリストのヘッダー末尾に出る。
一覧が更新されるのと同じタイミング（既定 3 秒ごと）で追随する。

```zsh
# 例: 自分の PR に付いた未対応のレビュー指摘を出す
export DECK_STATUS_CMD='zsh ~/src/devkit/scripts/pr-watch.sh status'
```

```
termdeck >
  Enter/ダブルクリック: 開く   ^N: 新規   ^K: 削除
  ^S: 再起動後に復元する/しない   ^R: 更新
  PR: termdeck#9 2件(P0/P1 1)      ← ここ
```

何を出すかは termdeck の関心事ではない。常に目に入る場所へ、外の状況を
1行だけ持ち込むための口として用意している。未設定なら何も足さない。
